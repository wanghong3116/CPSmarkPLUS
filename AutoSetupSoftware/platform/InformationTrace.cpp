...
CInformationTrace::CInformationTrace()
{
	...
	GetFullPathUnderExeW(FOLDER_LOG, m_szLogDir, _MAX_PATH);
}
...
int CInformationTrace::resetLogFilePath()
{
	...
	swprintf_s(m_szLogFile, _ARRAYSIZE(m_szLogFile), L"%s\\%d%02d%02d%02d%02d%02d%s", m_szLogDir,
		sysTime.wYear, sysTime.wMonth, sysTime.wDay,
		sysTime.wHour, sysTime.wMinute, sysTime.wSecond, m_szLogExt);
	...
	int nDeleteNum = (int)vstrDirAllFiles.size() - m_nLimitNum;
