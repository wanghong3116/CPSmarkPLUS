...
std::wstring DoubleToStringW(double dData, int nPercision)
{
	wstring strData = L"";
	wchar_t szData[MAX_PATH] = { 0 };
	wchar_t szPercison[_MAX_PATH] = { 0 };

	swprintf_s(szPercison, MAX_PATH, _T("%%.%dlf"), nPercision);
	swprintf_s(szData, MAX_PATH, szPercison,  dData);
	strData = szData;
...
int GetAllFilesUnderDirWithOutFilterA( const char* szDirFP, vector<string>* pvOutFiles, BOOL bChildOnly )
{
	...
	char  szFormatFile[MAX_PATH * 2]	= {0};
	...
	while (TRUE)
	{
		if (wfd.cFileName[0] != '.')
		{
			sprintf_s(szFile, ARRARY_COUNT(szFile), "%s\\%s", szDirFP, wfd.cFileName);
...
int GetAllFilesUnderDirWithOutFilterW( const wchar_t* szDirFP, std::vector<std::wstring>* pvOutFiles, BOOL bChildOnly )
{
	...
	wchar_t szFormatFile[MAX_PATH * 2]	= {0};
	...
	while (TRUE)
	{
		if (wfd.cFileName[0] != '.')
		{
			swprintf_s(szFile, ARRARY_COUNT(szFile), L"%s\\%s", szDirFP, wfd.cFileName);
			if (!(wfd.dwFileAttributes & FILE_ATTRIBUTE_DIRECTORY))
			{
...
int DeleteDirW( const wchar_t* szDirFP )
{
	...
	swprintf_s(szFormatFile, ARRARY_COUNT(szFormatFile), L"%s\\*.*", szDirFP);
	hFind = ::FindFirstFileW(szFormatFile, &wfd);
	if (INVALID_HANDLE_VALUE == hFind)
	{
		return -1;
...
BOOL FindProcessIDFromName(const TCHAR *szProcessName, std::vector<DWORD>& rvdwID)
{
	...
	while (Process32Next(hSnapShot, &pe))
	{
		...
		{
			rvdwID.push_back(pe.th32ProcessID);
		}
	}
	return FALSE;
}
...
typedef struct _tagEnumWindowProcParam  
{
	void *pClassName;
	void *pWndName;
	bool bParticalWndName;
	bool bParticalClassName;
...
)
{
	...
	if (pParam->bParticalClassName && pParam->bParticalWndName)
	{
		if (NULL != wcsstr(szCurWndClassName, szCalssName)
	...
	else if (pParam->bParticalWndName)
	{
		if (NULL != wcsstr(szCurWndWndName, szWndName)
			)
		{
			pvhwndFind->push_back(hwnd);
		}
	}
	else if (pParam->bParticalClassName)
	{
		if (NULL != wcsstr(szCurWndClassName, szCalssName)
			)
		{
...
)
{
	...
	if (pParam->bParticalClassName && pParam->bParticalWndName)
	{
		if (NULL != strstr(szCurWndClassName, szCalssName)
	...
	{
		if (NULL != strstr(szCurWndWndName, szWndName)
			)
		{
			pvhwndFind->push_back(hwnd);
		}
...
int FindWindowWithClassNameAndWindowNameW(const wchar_t *szClassName, const wchar_t* szWndName, bool bPartialClassName, bool bPartialWndName, std::vector<HWND> *pvhwndFind)
{
	...
	if (!bPartialWndName && !bPartialClassName)
	{
	...
	else
	{
		...
		param.bParticalClassName = bPartialClassName;
		param.bParticalWndName = bPartialWndName;
